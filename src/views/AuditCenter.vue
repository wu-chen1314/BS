<template>
    <div class="audit-center">
        ...
        <div class="audit-header">
            <div class="audit-title-area">
                ...
                <div>
                    ...
                    <p class="audit-subtitle">对申报的非遗项目进行审批管理，确保数据质量与规范性</p>
                </div>
            </div>
        ...
        <div class="filter-bar">
            <el-input v-model="searchName" placeholder="搜索项目名称..." :prefix-icon="Search"
                clearable @clear="fetchList" @keyup.enter="fetchList" style="width: 260px;" />
            <el-select v-model="searchLevel" placeholder="保护级别" clearable @change="fetchList" style="width: 140px; margin-left: 12px;">
        ...
        <el-table :data="tableData" v-loading="loading" border stripe
            ...
            <el-table-column label="项目名称" min-width="180">
                <template #default="{ row }">
                    <div class="project-name-cell">
                        <el-image v-if="row.coverUrl" :src="getFileUrl(row.coverUrl)"
                            class="mini-cover" fit="cover" />
                        <div class="mini-cover mini-placeholder" v-else>
            ...
            <el-table-column label="存续状态" width="90">
                <template #default="{ row }">
            ...
            <el-table-column v-if="activeTab === '2'" label="驳回原因" min-width="160">
                <template #default="{ row }">
            ...
            <el-table-column v-if="activeTab !== '0'" label="审核人" width="100">
                <template #default="{ row }">
                    <span class="text-muted">{{ row.auditBy || '—' }}</span>
                </template>
            </el-table-column>
        ...
        <div class="pagination-bar">
            <el-pagination v-model:current-page="currentPage" v-model:page-size="pageSize"
                ...
                @current-change="fetchList" @size-change="fetchList" background />
        </div>
            ...
            <el-form class="reject-form">
                <el-form-item label="驳回原因" required>
                    <el-input v-model="rejectReason" type="textarea" :rows="4"
                        placeholder="请详细说明驳回原因，将通知申报人..."
                        maxlength="200" show-word-limit />
                </el-form-item>
            </el-form>
            <template #footer>
                <el-button @click="rejectDialogVisible = false">取消</el-button>
                <el-button type="danger" :loading="auditLoading"
                    :disabled="!rejectReason.trim()"
                    @click="confirmReject">确认驳回</el-button>
            </template>
        </el-dialog>

        <!-- 项目详情抽屉 -->
        <el-drawer v-model="detailDrawerVisible" :title="currentProject?.name" size="520px">
            <div class="detail-content" v-if="currentProject">
                <el-image v-if="currentProject.coverUrl"
                    :src="getFileUrl(currentProject.coverUrl)"
                    class="detail-cover" fit="cover" />
                ...
                <el-descriptions :column="2" border size="small" class="detail-desc">
                    ...
                    <el-descriptions-item label="存续状态">{{ currentProject.status }}</el-descriptions-item>
                ...
                <div class="detail-history" v-if="currentProject.history">
                    <div class="detail-section-title">历史渊源</div>
                ...
                <div class="detail-actions" v-if="currentProject.auditStatus === 0 || currentProject.auditStatus === null">
                    <el-button type="success" @click="handleApprove(currentProject); detailDrawerVisible = false">
                        <el-icon><CircleCheck /></el-icon> 通过审核
                    </el-button>
...
import { Search, Refresh, Document, Clock, CircleCheck, CircleClose, WarningFilled, Picture, Select } from '@element-plus/icons-vue'
...
const CATEGORIES: Record<number, string> = {
    1: '民间文学', 2: '传统音乐', 3: '传统舞蹈', 4: '传统戏剧',
    5: '曲艺', 6: '传统体育、游艺与杂技', 7: '传统美术',
    8: '传统技艺', 9: '传统医药', 10: '民俗'
}

const getCategoryName = (id: number) => CATEGORIES[id] || '其他'
const getLevelType = (level: string) => {
    const m: any = { '国家级': '', '省级': 'success', '市级': 'warning', '县级': 'info' }
    return m[level] || 'info'
}
...
const getStatusIcon = (s: any) => {
    if (s === 1) return 'CircleCheck'
    if (s === 2) return 'CircleClose'
    return 'Clock'
}
    ...
    try {
        const res = await request.get('/projects/page', {
            params: {
                ...
                auditStatus: parseInt(activeTab.value)
            }
        })
...
const fetchStats = async () => {
    try {
        ...
        stats.pending = r0.data.data?.total || 0
        stats.approved = r1.data.data?.total || 0
        stats.rejected = r2.data.data?.total || 0
    } catch {}
}
...
.stat-pending { background: rgba(230, 162, 60, 0.2); border: 1px solid rgba(230,162,60,0.4); }
.stat-pass { background: rgba(103, 194, 58, 0.2); border: 1px solid rgba(103,194,58,0.4); }
.stat-reject { background: rgba(245, 108, 108, 0.2); border: 1px solid rgba(245,108,108,0.4); }
...
.project-meta { font-size: 11px; color: #909399; margin-top: 2px; }
...
.status-pending { background: #fdf6ec; color: #e6a23c; border: 1px solid #f5dab1; }
.status-approved { background: #f0f9eb; color: #67c23a; border: 1px solid #c2e7b0; }
